...
class FTP2:
    ...
    def __init__(self, host="sftp.crh.noaa.gov", port=0, ssl=True,ssl_params={"keyfile": "cert/key2.crt","certfile": "cert/cert2.crt"}, timeout=10):
        self.host = host
        self.port = port
    ...
    def download(self, host, port, timeout=10):
        print("File Host COMMAND: " + str(host))
        print("File Port COMMAND: " + str(port))
        self.sock = self._create_connection((host, port), timeout)
        return
